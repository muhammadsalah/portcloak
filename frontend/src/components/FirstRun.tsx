 ...
 *
 * Shared by Snapshots and Activity, because what is missing is the same in both
 * cases even though what is empty is not: an environment to read from, somewhere
 * to put what is read. The caller's own heading and opening sentence arrive on
 * the view; everything below them is common.
 *
 * Two numbered cards in the order the tool needs them, and the answer to the
 * question everyone asks first, which is whether PortCloak needs an account.
 */
    ...
    <Empty>
      ...
      <Cards>
        <Step
          ...
          onClick={() => navigate({ name: "environments", add: true })}
        />
        <Step
          number={2}
          ...
          onClick={() => navigate({ name: "storage", add: true })}
        />
      </Cards>
