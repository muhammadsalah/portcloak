...
import { KeyPrompt } from "./dialogs/KeyPrompt";
import { closeSnapshot, verify } from "./dialogs/actions";
...
export function InspectorPage({ route }: { route: InspectRoute }) {
  ...
  useEffect(() => {
    ...
    const open = async () => {
      ...
      modal.open({
        ...
        onCancel: () => navigate({ name: "snapshots" }),
      });
    };
  ...
  if (state.status === "failed") {
    return (
      <div>
        ...
        <Button style={{ marginTop: 12 }} onClick={() => navigate({ name: "snapshots" })}>
          <Icon name="back" />
          Back to snapshots
        </Button>
      </div>
  ...
  return (
    <div>
      <Breadcrumb>
        <Link onClick={() => navigate({ name: "snapshots" })}>Snapshots</Link>
      ...
      <PageHead>
        ...
        */}
        <Row>
          <Button onClick={() => void verify(route.snapshotId, modal)}>
            <Icon name="verify" />
            Verify
          </Button>
          <Button
