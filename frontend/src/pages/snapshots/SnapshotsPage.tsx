...
/** Snapshots is Tier 0: every snapshot, across every backend, with no key. */
...
export function SnapshotsPage() {
  ...
  if ("error" in state.value) {
    return (
      <Notice
        tone="danger"
        title="The snapshots could not be read."
        body={String(state.value.error)}
      />
    );
  }
  return <Snapshots view={state.value.view} reload={reload} />;
}

function Snapshots({ view, reload }: { view: LibraryView; reload: () => void }) {
